...
nDMaterial PM4Sand   1    $Dr  $G0  $hpo  $rho $Patm $h0  $emax $emin  $nb  $nd $Ado  $zmax  $cz   $ce	  $phicv  $nu_gravity $Cgd $Cdr $ckaf $Q $R $m_m $Fsed_min $p_sedo
...
# Create recorders
recorder Node  -nodeRange 1 4  -time -dT $recDT -file CycdispF.out  -dof 1 2 disp
recorder Node  -nodeRange 1 4  -time -dT $recDT -file CycpressF.out -dof 3 vel

# recorder Element -ele 1 -time -dT $recDT -file CycstressF.out material 1 stress
# recorder Element -ele 1 -time -dT $recDT -file CycstrainF.out material 1 strain

recorder Element -ele 1 -time -dT $recDT -file CycstressF.out stress
recorder Element -ele 1 -time -dT $recDT -file CycstrainF.out strain
# Create analysis
constraints Penalty 1e17 1e17
test        NormDispIncr 1.0e-5 35 0
...
set numCycle [getTime];
set nCyclesIO [open nCyclesResultF.dat a];
